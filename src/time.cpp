...
rsutil::timer::pause()
{
    ...
    dynamic_assert(_is_paused == false,
                   "[ rsutil::timer::pause() ] timer is already paused");
...
rsutil::timer::resume()
{
    ...
    dynamic_assert(_is_paused == true,
                   "[ rsutil::timer::resume() ] timer is not paused");
...
void
rsutil::scheduleInvocation(Func &&callable, const high_precision_float delayMs)
{
    StaticAssert(std::is_invocable_v<Func>, "Parameter callable must be invocable");

    const std::chrono::duration<high_precision_float, std::milli> delay(delayMs);
    std::jthread([callable = std::forward<Func>(callable), delay]
    {
